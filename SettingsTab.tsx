...
const SettingsTab: React.FC<Props> = ({ 
  ...
  const openEditModal = (card: Card) => {
    ...
    if (config?.merchantTypeRebates) {
      Object.entries(config.merchantTypeRebates as Record<string, number>).forEach(([key, val]) => {
              ...
              <div key={c.id} className="bg-white border border-[#e8e6e1] rounded-2xl p-4 minimal-shadow space-y-3">
                ...
                {!isCollapsed && (
                  <div className="space-y-3">
                    ...
                    {merchantTypes.map(m => <div key={m} className="flex justify-between items-center pl-2"><span className="text-[9px] text-[#717171]">{m}</span><input type="number" step="0.01" placeholder={config.basicRebate.toString()} value={config.merchantTypeRebates[m] ?? ''} onChange={e => updateRebate(c.id, m, e.target.value)} className="w-16 text-right border-b text-[#b4a088] outline-none text-[9px]" /></div>)}
                  </div>
                )}
              </div>
            );
